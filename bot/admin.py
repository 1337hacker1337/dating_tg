...
from bot.keyboards_admin import (
    ...
    kb_ad_channel, kb_ad_timer, kb_report_actions,
)
...
async def _show_report_page(call: CallbackQuery, session: AsyncSession, page: int) -> None:
    ...
    reason_val   = report.reason.value if hasattr(report.reason, "value") else report.reason
...
async def adm_reports_entry(call: CallbackQuery, session: AsyncSession):
    _log.info("adm:reports clicked by admin=%s", call.from_user.id)
    await call.answer()
    try:
        await _show_report_page(call, session, 0)
    except Exception as e:
        _log.error("adm_reports error: %s", e, exc_info=True)
        try:
            await call.message.answer(
...
async def adm_rep_dismiss(call: CallbackQuery, session: AsyncSession):
    ...
    try:
        ...
        pass
    # После отклонения total--; если были на последней странице — сдвигаемся назад
