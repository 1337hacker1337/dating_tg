...
from bot.states_admin import (
    ...
    AdminBroadcast, AdminCalibration, AdminAddAdmin,
)
...
async def adm_add_admin_exec(message: Message, state: FSMContext, session: AsyncSession):
    raw = (message.text or "").strip()
    if not raw.lstrip("-").isdigit():
        await message.answer("⚠️ Нужен числовой ID.")
        return
    ...
    await repo.add(telegram_id=target_id, username=username)
    await session.commit()
