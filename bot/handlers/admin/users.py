"""
bot/handlers/admin/users.py — поиск юзера, бан/разбан, калибровка,
...
def _admins_text(admins: list) -> str:
    if not admins:
        return "список пуст."
...
async def adm_lookup_handler(message: Message, state: FSMContext, session: AsyncSession):
    ...
    if add_mode:
        ...
        if await admin_repo.is_admin(target_id):
            ...
            return
        # БАГФИКС: заполняем added_by — раньше поле всегда оставалось NULL
        acting = await admin_repo.get_by_telegram_id(message.from_user.id)
        username = user.username if user else None
        await admin_repo.add(
            telegram_id=target_id, username=username,
            added_by=acting.id if acting else None,
        )
