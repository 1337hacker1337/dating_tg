...
async def _profile_caption(candidate: User, session, viewer=None) -> str:
    ...
    if viewer and viewer.latitude is not None and candidate.latitude is not None:
        dist = round(_haversine_km(viewer.latitude, viewer.longitude, candidate.latitude, candidate.longitude), 1)
        header += f"  ·  {dist} км"
    lines = [header]
    if candidate.bio:
    ...
    lines += ["", format_rating_line(candidate.avg_rating, candidate.rating_count), "",
...
async def _send_card(user_id, bot, candidate, session, viewer=None):
    ...
    if candidate.photos:
        await bot.send_photo(user_id, photo=candidate.photos[0].file_id, caption=caption, reply_markup=kb, parse_mode="HTML")
    else:
        await bot.send_message(user_id, caption + "\n\n<i>нет фото</i>", parse_mode="HTML", reply_markup=kb)

async def show_next(user_id, bot, session, delete_msg_id=None, viewer=None):
    if delete_msg_id:
        try: await bot.delete_message(user_id, delete_msg_id)
        except: pass
    ...
    if candidate is None:
        await bot.send_message(user_id, "пусто.\n\n<i>загляни позже.</i>", parse_mode="HTML", reply_markup=kb_main_menu()); return
...
async def handle_like(call: CallbackQuery, bot: Bot, session, db_user=None):
    ...
    if result.is_new_match and result.partner:
        ...
        if viewer:
            await _send_match_notification(result.partner.id, viewer, bot, session, write_to=call.from_user.id)
    elif result.notify_like:
        await _notify_liked(candidate_id, bot)
    await show_next(call.from_user.id, bot, session, delete_msg_id=call.message.message_id, viewer=db_user)
...
async def likes_page(call: CallbackQuery, bot: Bot, session):
    await call.answer()
    await _show_likes_page(call.from_user.id, bot, session, page=int(call.data.split(":")[1]), delete_msg_id=call.message.message_id)
...
async def _show_likes_page(user_id, bot, session, page, delete_msg_id=None):
    ...
    if total == 0:
        await bot.send_message(user_id, "лайков нет."); return
    page = max(0, min(page, total - 1))
    liker_id = await like_repo.get_unanswered_liker_at(user_id, page)
    if liker_id is None:
        await bot.send_message(user_id, "анкета удалена."); return
    repo = UserRepository(session)
    ...
    if page > 0: b.button(text="←", callback_data=f"likes_page:{page-1}")
    b.button(text=f"{page+1}/{total}", callback_data="noop")
    if page < total-1: b.button(text="→", callback_data=f"likes_page:{page+1}")
    ...
    b.adjust(3, 2)
    if liker.photos:
        await bot.send_photo(user_id, photo=liker.photos[0].file_id, caption=caption, reply_markup=b.as_markup(), parse_mode="HTML")
    else:
...
async def likes_react(call: CallbackQuery, bot: Bot, session):
    ...
    liker_id = int(liker_id_str); page = int(page_str); liked = action == "like"
    svc = BrowseService(session)
    ...
    if total == 0:
        ...
        await bot.send_message(call.from_user.id, "всё."); return
    await _show_likes_page(call.from_user.id, bot, session, page=min(page, total-1), delete_msg_id=call.message.message_id)
...
async def matches_page(call: CallbackQuery, bot: Bot, session):
    await call.answer()
    await _show_matches_page(call.from_user.id, bot, session, page=int(call.data.split(":")[1]), delete_msg_id=call.message.message_id)
...
async def _show_matches_page(user_id, bot, session, page, delete_msg_id=None):
    ...
    if not matches:
        await bot.send_message(user_id, "мэтчей нет."); return
    ...
    b = InlineKeyboardBuilder()
    if page > 0: b.button(text="←", callback_data=f"matches_page:{page-1}")
    b.button(text=f"{page+1}/{len(matches)}", callback_data="noop")
    if page < len(matches)-1: b.button(text="→", callback_data=f"matches_page:{page+1}")
    if partner.username:
    ...
    b.adjust(3, 1)
    if partner.photos:
        await bot.send_photo(user_id, photo=partner.photos[0].file_id, caption=caption, reply_markup=b.as_markup(), parse_mode="HTML")
    else:
        await bot.send_message(user_id, caption, reply_markup=b.as_markup(), parse_mode="HTML")
...
async def _send_match_notification(user_id, partner, bot, session, write_to=None):
    ...
    try:
        if partner.photos:
            await bot.send_photo(user_id, photo=partner.photos[0].file_id, caption=caption, reply_markup=kb, parse_mode="HTML")
        else:
...
async def _notify_liked(user_id, bot):
    try:
        await bot.send_message(user_id, "🩸  тебя лайкнули.\n<i>загляни в лайки.</i>", parse_mode="HTML")
    except Exception as e:
...
async def try_write(call: CallbackQuery): await call.answer("закрытый профиль.", show_alert=True)
