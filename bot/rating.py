"""
SMV-ранговая система на основе винрейта (лайки / всего оценок).
До MIN_VOTES оценок — режим калибровки, ранг скрыт.
"""
...
class Tier:
    slug:    str
    emoji:   str
    min_wr:  float   # минимальный винрейт (включительно)


MIN_VOTES = 50  # меньше — калибровка
...
def format_rating_line(avg_rating: float, rating_count: int) -> str:
    """
    До 50 оценок:  ⚙️ <i>Калибровка (12/50)</i>
    После:         🏆 [██████░░] 78% ⚡ htn
    """
    if rating_count < MIN_VOTES:
        return f"⚙️ <i>Калибровка ({rating_count}/{MIN_VOTES})</i>"
